...
class TestCodeCommenter(unittest.TestCase):
    ...
    def test_comment_with_overwrite(self):
        ...
        # Act
        result = self.commenter.comment(code=code, filename=filename, overwrite=True, language="python")

        # Assert
        self.mock_chain.invoke.assert_called_once()

        self.assertEqual(result, expected_commented_code)
    ...
    def test_comment_without_overwrite(self):
        ...
        # Act
        result = self.commenter.comment(code=code, filename=filename)
